...
def denoise_gaussian_helper(noisy_img, orig_img, num_nei, channel, l):
	Y = copy.deepcopy(noisy_img)
	height, width = Y.shape
	# burn in
	for b in range(1, MAX_BURNS+1):
	    gibbs_sampling_gaussian(noisy_img, Y, num_nei, l)
	    print 'burn in', b, ', channel', channel, psnr_mse2(Y, orig_img)
	# sampling
	posterior = np.zeros((height, width))
	for s in range(1, MAX_SAMPLES+1):
		gibbs_sampling_gaussian(noisy_img, Y, num_nei, l)
		print 'sampling', s, ', channel', channel, psnr_mse2(Y, orig_img)
		posterior += Y
...
def denoise_gaussian(noisy_img, orig_img, num_nei, l):
	...
	for ch in range(c):
		denoised[:,:,ch] = denoise_gaussian_helper(noisy_img[:,:,ch], orig_img[:,:,ch], num_nei, ch, l)
	return denoised
##################################################################################################
...
##################################################################################################
def denoise_sparse_grad_prior_helper(noisy_img, orig_img, channel, l_loss, l_grad, filter_size=3):
	Y = copy.deepcopy(noisy_img)
	height, width = Y.shape
...
def denoise_sparse_grad_prior(noisy_img, orig_img, l_loss, l_grad, filter_size=3):
	...
	denoised = np.zeros([h,w,c])
	for ch in range(c):
		denoised[:,:,ch] = denoise_sparse_grad_prior_helper(noisy_img[:,:,ch], orig_img[:,:,ch], ch, l_loss, l_grad, filter_size)
	return denoised
##################################################################################################
...
##################################################################################################
def denoise_ensemble_helper(noisy_img, orig_img, channel, sigma, filter_num=3, filter_size=3, smooth=True):
	Y = copy.deepcopy(noisy_img)
	height, width = Y.shape
	radius = (filter_size - 1) / 2
	...
	for s in range(1, MAX_SAMPLES+1):
		gibbs_sampling_ensemble(noisy_img, Y)
		print 'sampling', ', channel', channel, s, psnr_mse2(Y, orig_img)
		posterior += Y
	denoised = posterior / MAX_SAMPLES
	return denoised
...
def denoise_ensemble(noisy_img, orig_img, sigma, filter_num=3, filter_size=3, smooth=True):
	...
	denoised = np.zeros([h,w,c])
	for ch in range(c):
		denoised[:,:,ch] = denoise_ensemble_helper(noisy_img[:,:,ch], orig_img[:,:,ch], ch, sigma, filter_num, filter_size, smooth)
	return denoised
##################################################################################################
...
def add_noise(orig_img, sigma):
	h, w, c = orig_img.shape
	noisy_img = np.zeros([h,w,c])
	for ch in range(c):
		noisy_img[:,:,ch] = orig_img[:,:,ch] + np.random.normal(0, sigma, [h,w])
		#noisy_img[:,:,ch] = orig_img[:,:,ch] + (np.random.rand(h,w)-0.5)*sigma*2
...
def psnr_mse(noisy_img, orig_img):
	h, w, c = orig_img.shape
	mse = np.sum(np.square(noisy_img[1:h-1,1:w-1,:]-orig_img[1:h-1,1:w-1,:]))/(h-2)/(w-2)/c
	psnr = 10.*np.log10(np.amax(orig_img[1:h-1,1:w-1,:])**2 / mse)
	return psnr, mse

def psnr_mse2(noisy_img, orig_img):
	h, w = orig_img.shape
...
if __name__ == "__main__":
	# load original image
	img_name = 'birds'
	orig_img = cv2.imread(img_name+'.png', cv2.IMREAD_COLOR)
	height, width, channel = orig_img.shape
	...
	noisy_img_nlm = cv2.fastNlMeansDenoisingColored(np.array(noisy_img, dtype='uint8'))
